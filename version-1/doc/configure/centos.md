...
## Mlocate
Если на сервере будет много кеш файлов, удаляем mlocate (поиск файлов по маске используя заранее кешированую структуру файлов), т.к. он запускает по крону /usr/bin/updatedb, который создает большую нагрузку на диск и процессор во время обновлениыя базы файлов.
```bash
yum remove mlocate
```
